...
import "../../styles/weightField.scss";
...
/** Компонент, отображающий тип единиц изменения (вес/объем) и их количество. */
const WeightField = ({
    dimensionType,
    dimension,
    classEl
}: WeightFieldProps): React.ReactElement => {
    return (
        <div className={"weight " + classEl}>
            {dimensionType === "мл" && (
                <svg
                    width="9"
                    height="15"
            ...
            <span>{`${divideNumber(+dimension)} ${dimensionType}`}</span>
        </div>
    );
};
